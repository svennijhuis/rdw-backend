...
| 5 | **Partial/incorrect CSV never delivered** | After network transient, retry succeeds and complete CSV is returned; 5 retries exhausted → 502, no CSV sent | A single Socrata page times out and retries (exponential backoff 250ms → 4s + jitter) up to 5 times; success on retry 2 | Two pages fetched successfully, third page fails permanently (HTTP 404); request returns 502 without sending any CSV bytes; temp file cleaned up | Integration + Resilience | Mock Socrata with controlled failures; monitor temp file cleanup; verify no partial response sent on error |
